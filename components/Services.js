  ...
  return (
    <section id="services" className="py-14 sm:py-18 bg-white">
      <div className="container-px mx-auto max-w-7xl">
        <Reveal variant="fadeUp" className="max-w-3xl mx-auto mb-12 lg:mb-16 text-center">
        ...
        <StaggerParent className="services-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {SERVICES.map((s) => (
            <StaggerChild key={s.title}>
              <div
                style={{ background: s.bg, borderColor: s.color + '30', borderWidth: 1.5, borderStyle: 'solid' }}
                className="group relative rounded-2xl p-7 transition-all hover:shadow-xl hover:-translate-y-1 flex flex-col items-center text-center h-full"
              >
        ...
        <Reveal variant="fadeUp" delay={0.1} className="mt-14">
          ...
          <StaggerParent className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {LOAN_TYPES.map(({ label, sub }) => (
              <StaggerChild key={label} variant="scaleUp">
                <div className="rounded-xl bg-cream-50 border border-line p-4 text-center h-full">
        ...
        {/* Guarantee banner */}
        <Reveal variant="fadeUp" delay={0.05} className="mt-8">
          <div className="guarantee-banner rounded-2xl bg-lavender-700 px-6 py-4 flex items-center justify-center gap-3 text-cream-50 text-sm font-semibold text-center flex-wrap">
