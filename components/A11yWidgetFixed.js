...
function A11yIcon({ size = 52 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <radialGradient id="cg2" cx="38%" cy="32%" r="65%">
          <stop offset="0%" stopColor="#44b3a9" />
          <stop offset="55%" stopColor="#2a9d8f" />
          <stop offset="100%" stopColor="#1a635b" />
        </radialGradient>
        <radialGradient id="fg2" cx="40%" cy="30%" r="65%">
          <stop offset="0%" stopColor="#a9e2db" />
          <stop offset="100%" stopColor="#74ccc3" />
        </radialGradient>
      </defs>
      <circle cx="24" cy="24" r="22" fill="url(#cg2)" />
      <circle cx="24" cy="24" r="22" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1.5" />
      <circle cx="24" cy="13" r="3.5" fill="url(#fg2)" />
      <rect x="22.5" y="17" width="3" height="9" rx="1.5" fill="url(#fg2)" />
      <rect x="13" y="18.5" width="22" height="3" rx="1.5" fill="url(#fg2)" />
      <rect x="22.5" y="25.5" width="3" height="9.5" rx="1.5" transform="rotate(-15 22.5 25.5)" fill="url(#fg2)" />
      <rect x="22.5" y="25.5" width="3" height="9.5" rx="1.5" transform="rotate(15 25.5 25.5)" fill="url(#fg2)" />
      <ellipse cx="18" cy="14" rx="4.5" ry="2.5" fill="rgba(255,255,255,0.18)" transform="rotate(-20 18 14)" />
    </svg>
  );
}

export default function A11yWidgetFixed() {
  ...
  return (
    <>
      ...
      {/* Fixed above Deno bot (Deno is at bottom-5 = 20px, height ~72px, so we sit at 104px) */}
      <div style={{ position: 'fixed', bottom: 104, right: 20, zIndex: 9997 }}>
        <button
          ...
          <div role="dialog" aria-label="Accessibility settings" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={reset} title="Reset" style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', padding: 4 }}><RotateCcw size={13} /></button>
                <button onClick={() => setOpen(false)} aria-label="Close" style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', padding: 4 }}><X size={15} /></button>
              </div>
            </div>
